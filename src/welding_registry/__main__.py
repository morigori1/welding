...
def cmd_enrich(args: argparse.Namespace) -> int:
    ...
    def _coerce_birth_date(s: _pd.Series) -> _pd.Series:
        ...
        if base.isna().any():
            alt_parsed = s.map(lambda v: parse_jp_date(str(v)) if _pd.notna(v) else None)
            alt = _pd.to_datetime(alt_parsed, errors="coerce")
    ...
    try:
        ...
        if getattr(args, "workers_csv", None):
            ...
            print(
                "Workers not provided; use --workers-csv or populate DuckDB table 'workers'.",
                file=sys.stderr,
            )
            return 2
        ...
        if "birth_date" in wdf.columns:
            ...
            wdf["birth_year_west"] = wdf["birth_date"].map(
                lambda x: str(x)[:4] if isinstance(x, str) and len(x) >= 4 else ""
            )
        ...
        if "employee_id" in roster.columns and "employee_id" in wdf.columns:
            ...
            print(
                "No join key (employee_id/name) found to match roster with workers.",
                file=sys.stderr,
            )
            return 2
...
def cmd_ingest(args: argparse.Namespace) -> int:
    ...
    if "expiry_date" not in df.columns and args.expiry_from != "expiry_date":
        ...
        if src not in df.columns:
            print(
                f"Column '{args.expiry_from}' not present; cannot compute expiry.", file=sys.stderr
            )
            return 2
...
def cmd_ingest_vertical(args: argparse.Namespace) -> int:
    ...
    auto_cols = (
        getattr(args, "auto_person", False)
        or getattr(args, "auto_regno", False)
    ...
    for sheet in target_sheets:
        ...
        if (
            auto_cols
            or auto_blocks
            or str(person).upper() == "AUTO"
            or str(regno).upper() == "AUTO"
            or (not blks)
        ):
        ...
        if (
            ...
            try:
                ...
                if areas and "row_index" in df_i.columns:
                    ...
                    df_i["status"] = df_i[col].map(
                        lambda r: "active"
                        if (isinstance(r, (int, float)) and int(r) in rowset)
                        else "retired"
                    )
                    ...
                    if "used_col_min" in df_i.columns:
                        df_i["record_type"] = df_i["used_col_min"].map(
                            lambda c: "history"
                            if (isinstance(c, (int, float)) and int(c) >= pa_right)
                            else "current"
                        )
                    else:
    ...
    print(
        f"Ingested vertical blocks from sheets [{sheets_info}]. Rows: {len(df_roster)}. Wrote {outdir}"
    )
    return 0
...
def cmd_workers(args: argparse.Namespace) -> int:
    ...
    if missing:
        print(
            ...
            + ", ".join(missing)
            + ". You can also set DB_HOST, DB_NAME, DB_USER, DB_PASSWORD.",
            file=sys.stderr,
        )
        return 2
...
def cmd_review_persons(args: argparse.Namespace) -> int:
    ...
    try:
        if not con.execute(
            ...
            return 2
        df = con.execute(
            "SELECT name, COUNT(*) AS n FROM roster WHERE name IS NOT NULL GROUP BY name ORDER BY name"
        ).df()
        if (
...
def cmd_review_show(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        has_pos = bool(
            con.execute(
                "SELECT 1 FROM information_schema.columns WHERE table_name='roster' AND column_name='positions'"
            ).fetchone()
        )
        cols = (
            "name, license_no, qualification, "
            + ("positions, " if has_pos else "")
            + "first_issue_date, issue_date, expiry_date"
        )
...
def cmd_review_export(args: argparse.Namespace) -> int:
    ...
    try:
        df = con.execute(
            "SELECT name, license_no, qualification, first_issue_date, issue_date, expiry_date FROM roster"
        ).df()
    finally:
        con.close()
    store = ReviewStore(Path(args.review_db))
    ...
    if decs:
        ...
        out = (
            df.merge(ddf, how="left", on=["name_key", "license_no"])
            .drop(columns=["name_key"])
            .sort_values(["name", "qualification", "license_no"])
        )  # type: ignore
    else:
        out = df
    ...
    if "expiry_date" not in df.columns:
        ...
        if not src:
            ...
            if date_candidates:
                ...
                print(
                    "expiry_date not present and no source date column found to compute from.",
                    file=sys.stderr,
                )
                src = None
    ...
    if exp_missing:
        base_col = (
            ...
            if "issue_date" in df.columns
                ...
                else ("first_issue_date" if "first_issue_date" in df.columns else None)
            )
        )
        if base_col and "qualification" in df.columns:
            ...
            mask_boiler = (
                q.str.contains("ﾎﾞｲﾗ", na=False)
                | q.str.contains("ボイラ", na=False)
                | q.str.contains("boiler", na=False)
            )
            ...
            right = (
                df["expiry_date"] if "expiry_date" in df.columns else pd.Series([None] * len(df))
            )
            df["expiry_date"] = right.combine_first(
                exp_series.dt.date if hasattr(exp_series, "dt") else exp_series
            )
        ...
        if src and src in df.columns:
            dt = pd.to_datetime(df[src], errors="coerce")
            df["expiry_date"] = (
                dt + pd.DateOffset(years=int(getattr(args, "valid_years", 3)))
            ).dt.date
    ...
    if lic_sources:
        ...
        for src in lic_sources:
            ...
            if lic_path.is_file():
                ...
                f = scan_license_dir(
                    lic_path, debug=bool(getattr(args, "debug_ocr", False)), dump_dir=dump_dir
                )
        ...
        if not lic_df.empty:
            ...
            if "_lic_key" in merged.columns and "_lic_key" in lic_df.columns:
                merged = merged.merge(
                    lic_df[["_lic_key", "expiry_date", "issue_date"]].rename(
                        columns={
            ...
            if not did_merge and "_name_key" in merged.columns and "_name_key" in lic_df.columns:
                merged = merged.merge(
                    lic_df[["_name_key", "expiry_date", "issue_date"]].rename(
                        columns={
                            "expiry_date": "expiry_date_lic",
                            "issue_date": "issue_date_lic",
                        }
            ...
            if did_merge:
                if "expiry_date_lic" in merged.columns:
                    right = (
                        merged["expiry_date"]
                        if "expiry_date" in merged.columns
                ...
                if "issue_date_lic" in merged.columns and "issue_date" in merged.columns:
                    merged["issue_date"] = merged["issue_date_lic"].combine_first(
                        merged["issue_date"]
                    )
                df = merged
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pg.add_argument(
        "--active-by-print-area",
        dest="active_by_print",
        action="store_true",
        help="Mark rows within print area as status=active; outside as retired",
    )
    pg.add_argument(
        "--only-active-print",
        action="store_true",
        help="Filter output to rows within print area only",
    )
    ...
    pv = sub.add_parser(
        "ingest-vertical",
        help="Ingest vertical-block sheets; supports auto-detection of columns/blocks",
    )
    ...
    pv.add_argument(
        "--all-sheets",
        action="store_true",
        help="Process all P1/P2/... sheets (or all if none match)",
    )
    pv.add_argument(
        "--person", default="A", help="Column letter for name (default: A; use 'AUTO' to detect)"
    )
    pv.add_argument(
        "--regno",
        default="B",
        help="Column letter for registration number (default: B; use 'AUTO' to detect)",
    )
    ...
    pv.add_argument(
        "--block",
        action="append",
        help="Additional block as LABEL=RANGE (repeatable), e.g., --block JIS=C:H --block BOILER=I:K",
    )
    pv.add_argument(
        "--auto", action="store_true", help="Auto-detect person/regno columns and block ranges"
    )
    ...
    pv.add_argument(
        "--auto-blocks", action="store_true", help="Auto-detect block ranges (JIS/Boiler)"
    )
    pv.add_argument("--out", default="out", help="Output directory (default: out)")
    pv.add_argument(
        "--duckdb",
        help="Optional DuckDB path to write table 'roster' (default: env DUCKDB_DB_PATH)",
    )
    pv.add_argument(
        "--with-raw",
        action="store_true",
        help="Also export roster_raw.csv with raw block cell values",
    )
    pv.add_argument(
        "--active-by-print-area",
        dest="active_by_print",
    ...
    pdue.add_argument(
        "--duckdb",
        help="Optional path to DuckDB file to write result table 'due' (default: env DUCKDB_DB_PATH)",
    )
    ...
    pdue.add_argument(
        "--dump-ocr",
        dest="dump_ocr",
        help="Optional directory to write sanitized OCR snippets for debugging patterns",
    )
    pdue.add_argument(
        "--workers-csv",
        dest="workers_csv",
        help="Optional CSV of active workers to filter by (name/employee_id match)",
    )
    pdue.add_argument(
        "--debug-ocr",
        action="store_true",
        help="Print minimal OCR debug (no PII): match flags and text length",
    )
    ...
    px.add_argument(
        "--printer", default="DocuWorks PDF", help="Target printer (default: DocuWorks PDF)"
    )
    px.add_argument("--no-recurse", action="store_true", help="Do not recurse into subdirectories")
    px.add_argument(
        "--auto-enter",
        action="store_true",
        help="Windows only: auto-press Enter on Save dialogs during print",
    )
    px.add_argument(
        "--save-window-titles",
        default="名前を付けて保存;Save Print Output As;DocuWorks;CubePDF",
        help="Semicolon-separated window title hints to target when auto-pressing Enter",
    )
    px.add_argument("--viewer", help="Optional full path to dwviewer.exe/dwdesk.exe")
    px.set_defaults(func=cmd_xdw2pdf)

    pdts = sub.add_parser(
        "dates", help="List date-like tokens from a PDF and their normalized forms"
    )
    ...
    pe.add_argument(
        "--workers-csv",
        dest="workers_csv",
        help="Optional workers CSV (fallback to DuckDB table 'workers')",
    )
    ...
    paud.add_argument(
        "--include-rejected", action="store_true", help="Also list rejected candidates"
    )
    ...
    def _cmd_audit(args: argparse.Namespace) -> int:
        ...
        frames = []
        if root.is_file():
            df = audit_license_pdf(
                root, window=int(args.window), include_rejected=bool(args.include_rejected)
            )
            if not df.empty:
            ...
            for pth in sorted(root.rglob("*.pdf")):
                df = audit_license_pdf(
                    pth, window=int(args.window), include_rejected=bool(args.include_rejected)
                )
                if not df.empty:
    ...
    pw.add_argument(
        ...
        help="SQL Server host (e.g., 192.9.200.253\\OBCINSTANCE4)",
    )
    pw.add_argument("--db", dest="db", default=os.getenv("DB_NAME"), help="Database name")
    pw.add_argument(
        "--schema",
        dest="schema",
        default=os.getenv("DB_SCHEMA", "dbo"),
        help="Schema name (default: dbo)",
    )
    pw.add_argument(
        "--table",
        dest="table",
        default=os.getenv("DB_WORKER_TABLE", "T_TM_Worker_T"),
        help="Table name (default: T_TM_Worker_T)",
    )
    pw.add_argument("--user", dest="user", default=os.getenv("DB_USER"), help="DB username")
    pw.add_argument(
        "--password", dest="password", default=os.getenv("DB_PASSWORD"), help="DB password"
    )
    pw.add_argument(
        "--driver",
        dest="driver",
    ...
    pw.add_argument(
        "--encrypt", dest="encrypt", default=os.getenv("DB_ENCRYPT"), help="Encrypt=yes|no"
    )
    pw.add_argument(
        "--trust-server-certificate",
        dest="trust",
        default=os.getenv("DB_TRUST_SERVER_CERTIFICATE"),
        help="TrustServerCertificate=yes|no",
    )
    pw.add_argument(
        "--where", dest="where", help="Optional WHERE clause to filter active employees"
    )
    ...
    pw.add_argument(
        "--duckdb",
        help="Optional DuckDB path to write table 'workers' (default: env DUCKDB_DB_PATH)",
    )
    ...
    # --- Versioned snapshot storage (new) ---
    pv2 = sub.add_parser("ver", help="Versioned DB from periodic '資格一覧.xlsx' snapshots")
    pv2_sub = pv2.add_subparsers(dest="ver_cmd", required=True)

    pvsnap = pv2_sub.add_parser(
        "snapshot", help="Ingest one Excel snapshot and update validity intervals"
    )
    pvsnap.add_argument("xls", help="Path to 資格一覧.xlsx (or similar)")
    pvsnap.add_argument("--sheet", help="Optional sheet name/index (default: first)")
    pvsnap.add_argument("--date", help="Snapshot date YYYY-MM-DD (default: file mtime)")
    pvsnap.add_argument(
        "--duckdb", required=True, help="DuckDB database path for versioned storage"
    )

    def _cmd_vsnap(args):
        from .versioned import cli_snapshot
    ...
    pvsnap.set_defaults(func=_cmd_vsnap)

    pvasof = pv2_sub.add_parser("asof", help="List or export roster as of a given date")
    pvasof.add_argument("--date", required=True, help="As-of date YYYY-MM-DD")
    pvasof.add_argument("--duckdb", required=True, help="DuckDB database path")
    pvasof.add_argument(
        "--out", help="Output file path (.xlsx or .csv). If omitted, prints to console"
    )
    pvasof.add_argument("--format", choices=["xlsx", "csv"], default="xlsx")

    def _cmd_vasof(args):
        from .versioned import cli_asof
    ...
    pvasof.set_defaults(func=_cmd_vasof)

    pvdiff = pv2_sub.add_parser("diff", help="Show added/removed records between two dates")
    pvdiff.add_argument("--from", dest="date_from", required=True, help="Baseline date YYYY-MM-DD")
    pvdiff.add_argument("--to", dest="date_to", required=True, help="Compare-to date YYYY-MM-DD")
    pvdiff.add_argument("--duckdb", required=True, help="DuckDB database path")

    def _cmd_vdiff(args):
        from .versioned import cli_diff

        return cli_diff(args.date_from, args.date_to, duckdb=args.duckdb)
...
def cmd_xdw2pdf(args: argparse.Namespace) -> int:
    if not find_dwviewer():
        print(
            "DocuWorks Viewer not found (dwviewer.exe). Install DocuWorks Viewer/Desk and retry.",
            file=sys.stderr,
        )
        return 2
    root = Path(args.input)
    ...
    if getattr(args, "auto_enter", False):
        ...
        if helper is None:
            print(
                "--auto-enter requested but helper could not start (non-Windows or PowerShell unavailable). Continuing without it."
            )
